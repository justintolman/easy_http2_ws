...
export default class SocketHandler {
	...
	joinRoom(client, name){
		...
		room.broadcast({type:'join', sender:'room', msg:`${client.name||client.id} joined the room.`});
...
class Room {
	...
	addClient(client){
		...
		client.talk({history: this._messages});
	}
...
class Client {
	...
	set room(name){
		...
		this.log(`Client ${this.id} changed room to ${this._room.name}`);
	}

	talk(msg){
		let m = msg
		msg.sender = {id:this.id, name:this._name};
		// if(msg.direct) m.ddirect = direct;
